...
def read_part(part_id: int, db_session: Session = Depends(deps.get_db)) -> Part:
    ...
    Args:
        ...
        db_session (Session, optional): SQLAlchemy database session. Defaults to Depends(deps.get_db).
...
def create_part(part: PartBase, db_session: Session = Depends(deps.get_db)) -> Part:
    ...
    Raises:
        HTTPException: If there is an issue creating the part, such as validation errors (400 Bad Request).
    """
    try:    
...
def read_parts(skip: int = 0, limit: int = 10, db_session: Session = Depends(deps.get_db)) -> List[Part]:
    """
